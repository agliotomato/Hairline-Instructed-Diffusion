...
## 2. 공간 재샘플링 전략: Bilinear에서 Lanczos로
...
단순히 이미지를 줄이는 게 아니라, **"어떻게 줄여야 정보를 잃지 않을까?"**를 고민해야 했습니다.
...
결과는 성공적이었습니다. Lanczos를 적용하자 Latent Mask의 경계가 훨씬 날카롭게 유지되었고, 확산 모델이 머리카락 끝부분을 뭉개지 않고 선명하게 생성해내기 시작했습니다.


## 3. 경계 처리: "Smart Blur" 알고리즘

단순히 전체에 블러(Blur)를 주는 것은 정답이 아니었습니다. 블러를 많이 주면 머리카락이 풍성해지지만 디테일이 죽고, 조금 주면 디테일은 살지만 "가발"처럼 경계가 어색해졌기 때문입니다.
그래서 우리는 영역을 **두 가지(Core vs Edge)**로 나누어 다르게 처리하는 **Smart Blur** 전략을 세웠습니다.

### 3.1. 영역 분리 (Semantic Segmentation)
...
### 3.2. 이중 스케일 블러링 (Dual-Scale Blurring) & 마스크 합성
...
### 3.3. 마스크 강도 정규화 (Normalization) - *중요*
...
블러 후 가장 밝은 부분이 다시 **무조건 1.0(255)**이 되도록 스케일링을 해주었더니, **부드러운 경계(Soft Edge)는 유지하면서도 생성 신호(Conditioning)는 확실하게** 줄 수 있게 되었습니다.
...
### 4.1. 얼굴 색감이 왜 변할까?
Inpainting을 할 때 마스크가 없는 부분(얼굴, 배경)은 원래 그대로 나와야 한다고 생각하지만, 실제로는 VAE 인코딩/디코딩을 거치면서 **미세한 색감 변화(Spectrum Shift)**나 디테일 손실이 발생합니다. "분명 같은 사진인데 얼굴 톤이 묘하게 다른" 문제가 계속 발생했습니다.
...
이 한 줄의 수식을 통해 얼굴과 배경의 색감, 질감이 **100% 원본과 동일하게 유지**되면서, 생성된 머리카락만 자연스럽게 얹어지는 결과를 얻을 수 있었습니다.
