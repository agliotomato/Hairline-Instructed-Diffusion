...
def parse_args(input_args=None):
    parser = argparse.ArgumentParser(description="Train pixel-space ControlNet for Stage1 (bald conversion)")
    ...
    parser.add_argument("--output_dir", type=str, default="models/stage1_pixel")
...
def make_train_dataset(args, accelerator):
    ...
    if source_column not in column_names or target_column not in column_names:
        raise ValueError(f"Dataset must contain '{source_column}' and '{target_column}' columns")
...
def collate_fn(examples):
    ...
    return {"source_pixel_values": spv, "target_pixel_values": tpv}
...
def main(args):
    ...
    optimizer = torch.optim.AdamW(controlnet.parameters(), lr=args.learning_rate, betas=(args.adam_beta1, args.adam_beta2), weight_decay=args.adam_weight_decay, eps=args.adam_epsilon)
