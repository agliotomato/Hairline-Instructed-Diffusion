...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Training script for hairline conditioned diffusion v3 (Latent IdentityNet).")
    ...
    parser.add_argument("--output_dir", type=str, default="hairline_cond_v3")
...
def main():
    ...
    if accelerator.is_main_process and args.report_to.lower() != "none":
        accelerator.init_trackers("hairline_cond_v3", config=vars(args))
    ...
    # Initialize Latent IdentityNet
    # We initialize ControlNet BEFORE modifying UNet input channels, because IdentityNet takes 4-channel inputs (Latent + Proxy),
    # whereas Main UNet takes 5-channel inputs (Latent + Mask).
    if args.controlnet_model_name_or_path:
        logger.info(f"Loading pretrained Latent IdentityNet from {args.controlnet_model_name_or_path}...")
        if os.path.isdir(args.controlnet_model_name_or_path):
            ...
            logger.info(f"Loaded ControlNet weights. Missing: {len(missing)}, Unexpected: {len(unexpected)}")
    else:
    ...
    if accelerator.is_main_process:
        logger.info("***** Running training (v3 - Latent IdentityNet) *****")
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                # Latent IdentityNet Forward Pass
                # controlnet_cond = z_bald (Bald Proxy Latent)
                ...
                # Main UNet Forward Pass with Injection
                model_pred = unet(
                    unet_inputs,
                    timesteps,
                    encoder_hidden_states=encoder_hidden_states,
