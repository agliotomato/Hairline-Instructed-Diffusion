...
class ControlNetConditioningEmbedding(nn.Module):
    ...
    def __init__(
        ...
        self.conv_in = nn.Conv2d(conditioning_channels, block_out_channels[0], kernel_size=3, stride=1, padding=1)
...
class ControlNetModel(ModelMixin, ConfigMixin):
    ...
    def from_unet(
        ...
        if load_weights_from_unet:
            # conv_in_condition_weight = torch.zeros_like(controlnet.conv_in.weight)
            # conv_in_condition_weight[:, :4, ...] = unet.conv_in.weight
            # conv_in_condition_weight[:, 4:8, ...] = unet.conv_in.weight
            # controlnet.conv_in.weight = torch.nn.Parameter(conv_in_condition_weight)
            # controlnet.conv_in.bias = unet.conv_in.bias

            controlnet.conv_in.load_state_dict(unet.conv_in.state_dict())
            controlnet.conv_in_2.load_state_dict(unet.conv_in.state_dict())
    ...
    def forward(
        ...
        ## v1
        # controlnet_cond = torch.concat([sample, controlnet_cond], 1)
        # sample = self.conv_in(controlnet_cond)
